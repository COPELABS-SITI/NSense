 ...
 * Class is part of the NSense application. This class controls the User Interface and update it based on NSenseService results. 
 ...
 *
 */
public class NSenseActivity extends Activity {
	...
	/**
	 * This method bind the NSense service by passing the sensors details, and service connection
	...
	void doBindService(ArrayList<SensorProduct> sensors) {
		...
		if (sensors != null) {
			if (isMyServiceRunning(NSenseService.class)) {
				bindService(
						new Intent(NSenseActivity.this, NSenseService.class),
				 ...
				 * After the NSense finish, the bluetooth must be disable and
			...
			mBoundService.setOnStateChangeListener(new ChangeListener() {
				...
				public void onGetSociabilityChange(ArrayList<SocialityDetails> newSocial) {
					...
					if(siPercentage >= 20 && siPercentage <= 50){
						...
						final Toast tag = Toast.makeText(NSenseActivity.this, "You may become socially isolated",Toast.LENGTH_SHORT);
						...
						final Toast tag = Toast.makeText(NSenseActivity.this, devName + " is close to you",Toast.LENGTH_SHORT);
	...
	/**
	 * This method returns True/false if the NSense service is running/not
	 * @param nsenseService NSenseService class
	 * @return true/false 
	 */
	private boolean isMyServiceRunning(Class<NSenseService> nsenseService) {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		relative.setOnTouchListener(new OnSwipeTouchListener(this) {
			...
			public void onSwipeTop() {
				...
				super.onSwipeTop();
				if(listHistory != null){
					Intent intent = new Intent(NSenseActivity.this, NSenseHistoryActivity.class);
					intent.putParcelableArrayListExtra("cs.nsense", listHistory);

					startActivity(intent);

				} else {
					Toast.makeText(NSenseActivity.this, "History is being updated please wait 1 minute.", Toast.LENGTH_SHORT).show();
				}
			}
		...
		mSIAdapter = new SIAdapter(NSenseActivity.this, mSInteraction ,
				R.layout.listview_si_row, NSenseActivity.this);
		siListView.setAdapter(mSIAdapter);
		mSIAdapter.notifyDataSetChanged();
		Log.e(TAG, "isMyServiceRunning"
				+ isMyServiceRunning(NSenseService.class) + "");
		if (isMyServiceRunning(NSenseService.class)) {
			bindService(new Intent(NSenseActivity.this, NSenseService.class),
					mConnection, Context.BIND_AUTO_CREATE);
	...
	/**
	 * This method close the NSense service
	 */
	...
	/**
	 * This method start the NSense activity
	 */
	...
	/**
	 * This method stop the NSense activity
	 */
